...
"All Star": """
        ...
        WHERE ap.awardID = 'All-Star Game MVP' AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
    """,
"300+ HR CareerBatting": """
        SELECT playerID
        FROM (
            ...
            WHERE t.franchid in (select franchid from teams where teams.team_name = %s)  
            GROUP BY b.playerID
            HAVING total_hr >= 300
        ) AS career_hr;
    """,
    ...
    "40+ 2B SeasonBatting": """
        ...
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_2B >= 40;
    """,
    "40+ HR SeasonBatting": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID AND b.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_HR >= 40;
    """,
    "40+ Save SeasonPitching": """
        SELECT p.playerID
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID AND p.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_SV >= 40;
    """,
    "500+ HR CareerBatting": """
        SELECT playerID
        FROM (
    ...
    "Born Outside US 50 States and DC": """
        ...
        WHERE (birth_country NOT IN ('USA') OR birth_state NOT IN ('AL', 'AK', ..., 'WY', 'DC'))
          AND teamID IN (
              SELECT teamID FROM teams WHERE franchid in (select franchid from teams where teams.team_name = %s)  
          );
    """,
    "Canada": """
        SELECT playerID
    ...
    "Pitchedmin. 1 game": """
        ...
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID AND p.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_G > 0;
    """,
    "Played In Major Negro Lgs": """
...
JOIN teams t ON b.teamID = t.teamID
WHERE p.birthCountry = 'USA' AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
    """,
    "World Series Champ WS Roster": """
...
"30+ HR /30+ SB SeasonBatting": """
 ...
 "First Round Draft Pick": """
        ...
        WHERE d.round = 1 AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
    """,
"40+ WAR Career": """
    SELECT playerID
    FROM (
...
""",
    ...
    "6+ WAR Season": """
        ...
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID AND b.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_WAR >= 6;
    """,
    ".300+ AVG CareerBatting": """
        SELECT playerID
        FROM (
    ...
    ".300+ AVG SeasonBatting": """
        ...
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_H / b.b_AB > 0.300;
    """,
    "10+ HR SeasonBatting": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_HR >= 10;
    """,
    "10+ Win SeasonPitching": """
        SELECT p.playerID
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_W >= 10;
    """,
    "100+ RBI SeasonBatting": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_RBI >= 100;
    """,
    "100+ Run SeasonBatting": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_R >= 100;
    """,
    "20+ Win SeasonPitching": """
        SELECT p.playerID
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_W >= 20;
    """,
    "200+ Hits SeasonBatting": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_H >= 200;
    """,
    "200+ K SeasonPitching": """
        SELECT p.playerID
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_SO >= 200;
    """,
    "200+ Wins CareerPitching": """
        SELECT playerID
        FROM (
...
""",
    ...
    "Cy Young": """
      ...
      JOIN appearances a ON ap.playerID = a.playerID AND ap.yearID = a.yearID
      JOIN teams t ON a.teamID = t.teamID AND a.yearID = t.yearID
      WHERE ap.awardID = 'Cy Young Award' AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
  """,
    ...
    "Gold Glove": """
      ...
      JOIN appearances a ON ap.playerID = a.playerID AND ap.yearID = a.yearID
      JOIN teams t ON a.teamID = t.teamID AND a.yearID = t.yearID
      WHERE ap.awardID = 'Gold Glove' AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
  """,
    ...
    "MVP": """
      ...
      JOIN appearances a ON ap.playerID = a.playerID AND ap.yearID = a.yearID
      JOIN teams t ON a.teamID = t.teamID AND a.yearID = t.yearID
      WHERE ap.awardID = 'Most Valuable Player' AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
  """,
    ...
    "Rookie of the Year": """
      ...
      JOIN appearances a ON ap.playerID = a.playerID AND ap.yearID = a.yearID
      JOIN teams t ON a.teamID = t.teamID AND a.yearID = t.yearID
      WHERE ap.awardID = 'Rookie of the Year' AND t.franchid in (select franchid from teams where teams.team_name = %s)  ;
  """,
    ...
    "Silver Slugger": """
      ...
      WHERE ap.awardID = 'Silver Slugger' AND t.franchid in (select franchid from teams where teams.team_name = %s)   AND ap.yearid = t.yearid and a.yearid = t.yearid and ap.playerid != 'stantmi03';
  """,
    ...
    "Hall of Fame": """
      ...
      JOIN teams t ON a.teamID = t.teamID
      WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND h.inducted = 'Y';
  """,
    ...
    "Threw a No-Hitter": """
      ...
      FROM pitching p
      JOIN teams t ON p.teamID = t.teamID AND p.yearID = t.yearID
      WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_NO != 0;
  """,
    ...
    "30+ HR / 30+ SB SeasonBatting": """
        ...
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_HR >= 30 AND b.b_SB >= 30;
    """,
    "30+ HR SeasonBatting": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_HR >= 30;
    """,
    "30+ SB Season": """
        SELECT b.playerID
        FROM batting b
        JOIN teams t ON b.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND b.b_SB >= 30;
    """,
    "30+ Save SeasonPitching": """
        SELECT p.playerID
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_SV >= 30;
    """,
    "Played Catchermin. 1 game": """
        SELECT f.playerID
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = 'C' AND f.f_G > 0;
    """,
    ...
    "Played First Basemin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = '1B' AND f.f_G > 0;
    """,
    ...
    "Played Second Basemin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = '2B' AND f.f_G > 0;
    """,
    ...
    "Played Third Basemin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = '3B' AND f.f_G > 0;
    """,
    ...
    "Played Shortstopmin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = 'SS' AND f.f_G > 0;
    """,
    ...
    "Played Left Fieldmin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = 'LF' AND f.f_G > 0;
    """,
    ...
    "Played Center Fieldmin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = 'CF' AND f.f_G > 0;
    """,
    ...
    "Played Right Fieldmin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = 'RF' AND f.f_G > 0;
    """,
    ...
    "Played Outfieldmin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position IN ('LF', 'CF', 'RF') AND f.f_G > 0;
    """,
    ...
    "Designated Hittermin. 1 game": """
        ...
        FROM fielding f
        JOIN teams t ON f.teamID = t.teamID AND f.yearID = t.yearID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND f.position = 'DH' AND f.f_G > 0;
    """,
    "≤ 3.00 ERA CareerPitching": """
        SELECT playerID
        FROM (
    ...
    "≤ 3.00 ERA Season": """
        ...
        FROM pitching p
        JOIN teams t ON p.teamID = t.teamID
        WHERE t.franchid in (select franchid from teams where teams.team_name = %s)   AND p.p_ER / (p.p_IPOuts / 3) <= 3.00;
    """,
"Only One Team": """
        SELECT a.playerID
        FROM appearances a
        ...
        HAVING SUM(CASE WHEN a.teamID NOT IN (
            ...
            FROM teams
            WHERE franchid in (select franchid from teams where teams.team_name = %s)  
        ) THEN 1 ELSE 0 END) = 0;
    """,
}
